...
const state = {
  mode:"normal", products:[], reviews:null, news:null, config:null,
  currentProductId:"shiromimi", showingBack:false, anomaly3Clicks:0, noiseNextMode:null
};
const els = {
  body: document.body,
...
window.state = state; window.els = els;
...
function bindImageFallback(img, fallback) {
  ...
  img.addEventListener("error", () => {
    if (state.config?.useLocalFallback && img.src !== new URL(fallback, location.href).href) img.src = fallback;
  });
}
...
function currentProduct() { return state.products.find(p => p.id === state.currentProductId) || state.products[0]; }
...
function getBackImage(product) {
  if (product.id === "shiromimi") {
    if (state.mode === "normal" || state.mode === "anomaly1") return product.anomaly1BackImage || product.backImage || product.image;
...
function runSiteAlteredOverlay(nextMode = null) {
  if (!els.noiseOverlay) {
    if (typeof nextMode === "string" && nextMode) { updateUrlMode(nextMode); setMode(nextMode); }
    return;
  }
...
function setMode(mode) {
  ...
  renderHeaderAndHero(); renderProducts(); renderDetail(); renderNews(); renderReviews(); renderBrand();
}
...
function renderDetail() {
  ...
  if (els.detailDescription) els.detailDescription.textContent = state.mode === "truth" ? (product.craftTruth || product.description || "") : (product.description || "");
  ...
  if (els.detailCraft) {
    if (state.mode === "anomaly2") els.detailCraft.textContent = product.anomaly2Craft || "対象者を内部へ押し込み、外見が崩れないよう縫合線を再調整します。";
    else if (state.mode === "anomaly3" || state.mode === "truth") els.detailCraft.textContent = product.craftTruth || product.craftNormal || "";
    else els.detailCraft.textContent = product.craftNormal || "";
  }
  ...
  if (els.craftPanel) { els.craftPanel.classList.add("hidden"); els.craftPanel.style.display = "none"; }
}
function renderNews() {
...
function bindEvents() {
  ...
  if (els.toggleBackBtn) {
    els.toggleBackBtn.addEventListener("click", e => { e.preventDefault(); state.showingBack = !state.showingBack; renderDetail(); });
  }
  if (els.detailImage) {
    els.detailImage.addEventListener("click", () => {
      const product = currentProduct();
      if (!product || !state.showingBack) return;
      if (product.id === "shiromimi" && (state.mode === "normal" || state.mode === "anomaly1")) runSiteAlteredOverlay("anomaly2");
    });
  }
  if (els.detailCraft) {
    els.detailCraft.addEventListener("click", () => { if (state.mode === "anomaly2") runSiteAlteredOverlay("anomaly3"); });
  }
  if (els.detailBirthplace) {
    els.detailBirthplace.addEventListener("click", () => {
      if (state.mode !== "anomaly3") return;
      if (state.anomaly3Clicks < 5) { state.anomaly3Clicks += 1; renderDetail(); return; }
  ...
  if (els.searchForm) {
    els.searchForm.addEventListener("submit", e => {
      ...
      if (!q) { if (els.searchMessage) els.searchMessage.textContent = "検索語を入力してください。"; return; }
      const product = state.products.find(p => (p.name || "").includes(q) || (p.id || "").includes(q));
      if (product) {
        state.currentProductId = product.id; state.showingBack = false; state.anomaly3Clicks = 0;
        renderProducts(); renderDetail();
        const detail = document.getElementById("detail");
        if (detail) detail.scrollIntoView({behavior:"smooth", block:"start"});
...
window.checkRequestedState = function () {
  const product = currentProduct();
  return {
    mode: state.mode, currentProductId: state.currentProductId, showingBack: state.showingBack,
    anomaly3Clicks: state.anomaly3Clicks, backImage: product ? getBackImage(product) : null,
...
window.inspectOverlayDom = function () {
  return {
    ...
    strayNoiseLayers: document.querySelectorAll('body > .noise-layer, body > .noise-flash, body > .noise-message').length,
